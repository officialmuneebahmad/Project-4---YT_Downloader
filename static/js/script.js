    ...
    try {
      ...
      eventSrc.onmessage = (e) => {
        ...
        if (info.status === "downloading") {
          ...
          progressInner.style.width = "97%";
          downloadBtn.textContent = "Uploading...";
        } else if (info.status === "finished" || info.status === "done") {
          progressInner.style.width = "100%";
          downloadBtn.textContent = "Opening link...";
          eventSrc.close();

          setTimeout(() => {
            if (info.download_link) {
              const a = document.createElement("a");
              a.href = info.download_link;
              a.target = "_blank";
              a.click();
              statusEl.textContent = "✅ File ready on Gofile!";
            } else {
              statusEl.textContent = "❌ Upload failed — no link returned.";
            }
...
// Perfectly Working local + online js file
